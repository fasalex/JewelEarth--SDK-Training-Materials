...
using Joa.JewelEarth.Basics.Events;
...
namespace JewelAddinTraining.UI.Windows.Logic.ViewModels
{
    public class CreatePolylineSetViewModel : OperationViewModel
    {
        ...
        private readonly IJewelExplorerUiService m_jewelExplorerUiService;

        public CreatePolylineSetViewModel(IFactoryService factoryService,
                                          IUiService uiService,
                                          IJewelExplorerUiService jewelExplorerUiService)
        {
            m_factoryService = factoryService;
            m_uiService = uiService;
            m_jewelExplorerUiService = jewelExplorerUiService;
        ...
        /// <summary>
        /// This method is called to perform a task when this action is invoked from the UI.
        /// </summary>
        private void OnExecute()
        {
            ...
            createPolylineSetCommand.Name = @"Test PolylineSet";
            ...
            var createdItem = createPolylineSetCommand.CreatedItem;

            HighlightInJewelExplorer(createdItem);
        }

        private void HighlightInJewelExplorer(IItem item)
        {
            if (item != null)
            {
                m_uiService.RaiseUiUpdate(UiUpdateEventArgs.Empty);
                m_jewelExplorerUiService.SelectModelItems(new[] { item }, true);
                m_jewelExplorerUiService.ExpandToModelItem(item);
            }
        }
    }
}